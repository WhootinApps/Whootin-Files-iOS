//
//  WMFSecondChooseLocationViewController.m
//  WHOOTIN:-)
//
...
@interface WMFSecondChooseLocationViewController ()
...
@implementation WMFSecondChooseLocationViewController
static int didcount;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(indexPath.section==0)
    {
        if(indexPath.row==0)
        {
            ...
            gallery_name.text = @"Up to Backup Files";
...
- (IBAction)btn_add_contentValues:(id)sender
{
    ...
    popview.allFilesUploads = @"alls";
    [self ValueInserted:_dataNameValues];
    ...
    dispatch_async(concurrentQueue, ^{
        dispatch_async(dispatch_get_main_queue(), ^{*/
             ...
             [self whootinSingleFileUploads:_dataValues fileName:_dataNameValues];
...
- (IBAction)btn_folder_creations:(id)sender
{
    ...
    viw_folderCreations.hidden = NO;
    
}
-(void)whootinsFolderCreation
{
    NSString *sUserDefault =kAccessToken;
    ...
    [request setHTTPMethod:@"POST"];
    NSLog(@"body made");
    NSString *fl = [NSString stringWithFormat:@"%@",_txt_view_folder.text];
    //NSString *fl = @"FoldeInner";
    ...
    NSString *pid = [NSString stringWithFormat:@"%@",[json valueForKey:@"parent_id"]];
    
    NSLog(@"Parent_id:%@",pid);
    [self pathStorageUsesDB:[NSString stringWithFormat:@"%@",[json valueForKey:@"name"]] IDS:_idValuesStore];
     [self backStorageUsesDBs:_idValuesStore IDS:pid BOOLCHECKING:[self countDataFromDBBack:_idValuesStore]];
...
-(void)whootinSingleFileUploads:(NSMutableData *)img fileName:(NSString *)fnames
{
    NSString *sUserDefault =kAccessToken;
    NSURL *postURL = [[NSURL alloc] initWithString:@"http://whootin.com/api/v1/files/new.json"];
    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:postURL
...
{
    viw_folderCreations.hidden = YES;
}
- (IBAction)btn_yes:(id)sender
{
    [self whootinsFolderCreation];
    viw_folderCreations.hidden = YES;
}
- (IBAction)btn_cancel_mainView:(id)sender
{
    [self performSegueWithIdentifier:@"steponeCancel" sender:self];
}
- (IBAction)returnToStepPopSecondViews:(UIStoryboardSegue *)segue
{
