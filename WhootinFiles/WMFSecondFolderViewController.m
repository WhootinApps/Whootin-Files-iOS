//
//  WMFSecondFolderViewController.m
//  WHOOTIN:-)
//
...
@interface WMFSecondFolderViewController ()
...
@implementation WMFSecondFolderViewController
static int didcount;
...
- (void)viewDidLoad
{
    ...
    {
        viw_Upgrade.hidden=NO;
    }
    else
    {
        viw_Upgrade.hidden=YES;
...
{
    NSLog(@"HHHHHHHHHHJJJJJJJKKKKKK:%@",self.idValue);
    
    if ([_createornot isEqualToString:@"bhuvan"])
    {
    ...
    // getting an NSString
    NSString *myString = [prefs stringForKey:@"upgrades"];
    NSString *locks = [prefs stringForKey:@"lock"];
    
    if([locks isEqualToString:@"lock"])
    {
        viw_CreatedFolder.hidden=YES;
...
- (void)updateCurrentTime
{
    if(UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone)
    {
        ...
        [UIView transitionWithView:_img_upgrade duration:20 options:UIViewAnimationOptionTransitionCrossDissolve   animations:^{
            _img_upgrade.image = [UIImage imageNamed:@"upgradecancel-20GB.png"];
        } completion:^(BOOL done){
            [UIView transitionWithView:_img_upgrade duration:20 options:UIViewAnimationOptionTransitionCrossDissolve animations:^{
                _img_upgrade.image = [UIImage imageNamed:@"upgradecancel-100GB.png"];
            } completion:^(BOOL done){
                [UIView transitionWithView:_img_upgrade duration:20 options:UIViewAnimationOptionTransitionCrossDissolve animations:^{
                    
                    _img_upgrade.image = [UIImage imageNamed:@"upgradecancel-1TB.png"];
    ...
    {
        [UIView transitionWithView:_img_upgrade duration:20 options:UIViewAnimationOptionTransitionCrossDissolve   animations:^{
            _img_upgrade.image = [UIImage imageNamed:@"ipad_upgrade20_cancel.png"];
        } completion:^(BOOL done){
            [UIView transitionWithView:_img_upgrade duration:20 options:UIViewAnimationOptionTransitionCrossDissolve animations:^{
                _img_upgrade.image = [UIImage imageNamed:@"ipad_upgrade100_cancel.png"];
            } completion:^(BOOL done){
                [UIView transitionWithView:_img_upgrade duration:20 options:UIViewAnimationOptionTransitionCrossDissolve animations:^{
                    
                    _img_upgrade.image = [UIImage imageNamed:@"ipad_upgrade1tb_cancel.png"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (tableView==_tableViewPaths)
    {
        ...
        cell.textLabel.text=[[mu_dirnames objectAtIndex:indexPath.row]objectForKey:@"fname"];
        ...
        NSLog(@"ValuesKKKKK::::::KK%@",[[mu_dirnames objectAtIndex:indexPath.row]objectForKey:@"rowid"]);
  ...
  //  NSLog(@"Rows_Id didcount :%d",didcount);
    ...
    if([mu_backmovefolders count]>0)
    {
        ...
        //NSLog(@"ReverseObjectEnumerator:%@",[[reversedArray objectAtIndex:i] objectForKey:@"parentidv"]);
            ...
            if (([mu_backmovefolders count]-1)==i)
            {
                _checkinsert = @"finsihed";
            }
        }
...
-(BOOL)FolderNameChecking:(NSString *)values
{
    BOOL isValid=false;
    NSString *status;
    const char *dbpath = [databasePath UTF8String];
...
-(BOOL)FolderNameCheckingss:(NSString *)values names:(NSString *)fileNames
{
    BOOL isValid=false;
    NSString *status;
    NSString *names;
    ...
    if (sqlite3_open(dbpath, &contactDB) == SQLITE_OK)
    {
        NSString *querySQL = [NSString stringWithFormat:@"SELECT whootin_name,whootin_type FROM whootinsecond_tbl where whootin_name=\"%@\" AND whootin_type=\"%@\"",values,fileNames];
        const char *query_stmt = [querySQL UTF8String];
...
-(void)facebooked
{
    ...
    if([SLComposeViewController isAvailableForServiceType:SLServiceTypeFacebook])
    {
        ...
        [controller setInitialText:_strs];
...
-(void)twitterpost
{
    if ([SLComposeViewController isAvailableForServiceType:SLServiceTypeTwitter])
    {
        ...
        [tweetSheet setInitialText:_strs];
...
{
    CGRect frame;
    if ([textField isEqual:_txt_folderUpload])
    {
        if(UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone)
        {  CGSize result = [[UIScreen mainScreen] bounds].size;
            if(result.height==568)
            {
                frame=[viw_CreatedFolder frame];
                frame.origin.y=482;
            }
            else
            {
        ...
        [self slideUpDown:viw_CreatedFolder withFrame:frame];
    }
...
- (IBAction)btn_ok_folderUploads:(id)sender
{
   ...
   BOOL folderNamesChecking= [self FolderNameCheckingss:[NSString stringWithFormat:@"%@",_txt_folderUpload.text] names:@"folder"];
    if (fl.length!=0)
    {
        ...
        {
        [self whootinFilesFolders];
        viw_CreatedFolder.hidden=YES;
        _txt_folderUpload.text=@"";
        }
    }
    else
    {
    ...
    [_txt_folderUpload resignFirstResponder];
    [self slideUpDown:viw_CreatedFolder withFrame:frame];
}
- (IBAction)btn_Gallary_Upload:(id)sender
{
    //[viw_CreatedFolder removeFromSuperview];
    viw_CreatedFolder.hidden=YES;
}
...
-(void)whootinFilesFolders
{
    ...
    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:postURL
                                                       ...
                                                       timeoutInterval:30.0];
    NSLog(@"Post Comment is[[[[>>>>>>>>>]]]]:%@",_fileIdFormtb);
    NSString *getFolderIds;
    
        if([_fileIdFormtb length]==0)
    {
        getFolderIds =  [NSString stringWithFormat:@"%@",self.idValue];
    }
    else
    {
        getFolderIds = [NSString stringWithFormat:@"%@",_fileIdFormtb];
    }
    NSLog(@"Post Comment is[[[[>>>>>>>>>]]]]:%@",getFolderIds);
    ...
    NSLog(@"body made");
    NSString *fl = [NSString stringWithFormat:@"%@",_txt_folderUpload.text];
    NSString *params=[NSString stringWithFormat:@"name=%@&folder_id=%@",fl,getFolderIds];
    ...
    //[self pathStorageUsesDB:[NSString stringWithFormat:@"%@",fl]];
    self.idValue = [NSString stringWithFormat:@"%@",[json valueForKey:@"id"]];
    _fileIdFormtb=self.idValue;
   [self whootingGetFiles:self.idValue];
    [self backFolderStorageUsesDBs:[NSString stringWithFormat:@"%@",[json valueForKey:@"id"]] IDS:[NSString stringWithFormat:@"%@",[json valueForKey:@"parent_id"]] BOOLCHECKING:[self countDataFromDBBackFolder:[NSString stringWithFormat:@"%@",[json valueForKey:@"id"]]]];
   [self pathStorageUsesDB:[NSString stringWithFormat:@"%@",fl] IDS:@"0"];
   [self.tableView reloadData];
   [_tableViewPaths reloadData];
   //[self.view makeToast:@"Successfully folder created"
               // duration:2.0
    ...
    if (sqlite3_open(dbpath, &contactDB) == SQLITE_OK)
    {
        ...
        if(sqlite3_prepare_v2(contactDB, query_stmt, -1, &statement, NULL) == SQLITE_OK)
        {
            while (sqlite3_step(statement) == SQLITE_ROW)
            {
                ...
                [temp setObject:rowidv forKey:@"rowid"];
